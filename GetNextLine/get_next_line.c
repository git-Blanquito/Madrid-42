...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/06 00:08:23 by aiblanco          #+#    #+#             */
/*   Updated: 2023/08/31 21:08:04 by imurugar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	stc_str = new_stc_str(fd, stc_str);
	if (!stc_str)
		return (NULL);
	line = get_line2(stc_str);
	stc_str = clean_stc_str(stc_str);
	return (line);
}
